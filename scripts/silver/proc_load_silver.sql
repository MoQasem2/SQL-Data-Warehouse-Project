...
BEGIN
	...
	CASE WHEN UPPER(TRIM(cst_marital_status)) ='S' THEN 'Singel'
	...
	SET @start_time =GETDATE();					  -----INSERTING INTO----crm_prd_info---
	...
	CASE WHEN sls_price IS NULL OR sls_price <=0 
		...
		ELSE sls_price
	END  AS sls_price   -----Derive price if original values if invalid
	...
	CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid,4,LEN(cid))
		ELSE cid ---------Remove 'NAS' prefix if present in 
	END cid,
	...
	CASE WHEN UPPER(TRIM(gen)) IN ('F','FEMALE') THEN 'Female'
		 ...
		 ELSE 'n/a'  ----Normalize gender values and handle unkown cases (empty value to null)
	End AS gen
